...
def MySpectral (im, imageType, numClusts):
    ...
    width = im.shape[1]
    bands = im.shape[2]
    print (f'image size is: {height}  {width}  {bands}')
    ...
    # hyperspectral images
        ...
        im_reduced = im_change
        print (im_reduced.shape)

        imh = im_reduced.reshape(height, width, bands)
        print(imh.shape)

        org_size = imh.shape
        # im_small = resize(imh, (122, 68))
        im_small = imh
        shk_size = im_small.shape[0:2]
        im_input = im_small.reshape(shk_size[0] * shk_size[1], bands)
